...
class Game:
    ...
    def load_data(self): #helps initialize game
        ...
        self.map_data = []
        self.title_font=pg.font.match_font("lucidafax", 12)
    ...
    def new(self): #helps initialize game
        ...
        for row, tiles in enumerate(self.map_data):
            for col, tile in enumerate(tiles):
                if tile == '1':
                    sprites1.Wall(self, col, row)
                if tile == "2":
                    sprites1.Table(self, col, row)
                if tile == "E":
                    self.empl=sprites1.Employee(self, col, row)

                if tile == 'P':
                    self.player= sprites1.Player(self, col, row)
